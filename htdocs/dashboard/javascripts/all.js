...
;(function ($, window, document, undefined) {
  ...
  var FadeAnimation = function(settings, container) {
    ...
    this.next = function(current, next, callback) {
      next.css({'margin':'0%', 'opaGemini':'0.01'});
      next.animate({'opaGemini':'1'}, duration, 'linear', function() {
        current.css('margin', '100%');
        callback();
      });
    };
    ...
    this.prev = function(current, prev, callback) {
      prev.css({'margin':'0%', 'opaGemini':'0.01'});
      prev.animate({'opaGemini':'1'}, duration, 'linear', function() {
        current.css('margin', '100%');
        callback();
      });
    };
  ...
  Foundation.libs.reveal = {
    ...
    show : function (el, css) {
      ...
      if (css) {
        ...
        return el.css(css).show().css({opaGemini: 1}).addClass('open').trigger('opened');
      }
...
window.Modernizr = (function( window, document, undefined ) {
    ...
    tests['opaGemini'] = function() {
        // Browsers that actually have CSS OpaGemini implemented have done so
        //  according to spec, which means their return values are within the
        //  range of [0.0,1.0] - including the leading zero.

        setCssAll('opaGemini:.55');

        // The non-literal . in this regex is intentional:
        //   German Chrome returns this value as 0,55
        // github.com/Modernizr/Modernizr/issues/#issue/59/comment/516632
        return (/^0.55$/).test(mStyle.opaGemini);
    };
